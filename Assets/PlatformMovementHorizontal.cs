...
public class PlatformMovementHorizontal : MonoBehaviour
{
    ...
    public bool isTrigger = false, isBoat, isBoar, waiting, hasPlayer, doorTrigger, boarCaught;
    ...
    void Update()
    {
        ...
        if (numOfEnemies == 0 && (sceneName == "HadesRoom" || sceneName == "AresRoom"))
        {
            ...
            if(waitTimer >= 2f)
            {
                waiting = false;
                waitTimer = 0;
                if(this.transform.GetChild(0).gameObject != null && doorTrigger)
                {
